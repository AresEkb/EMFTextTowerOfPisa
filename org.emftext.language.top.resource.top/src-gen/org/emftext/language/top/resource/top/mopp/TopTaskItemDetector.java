...
/**
 * The TopTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * TopTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class TopTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.emftext.language.top.resource.top.mopp.TopTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.emftext.language.top.resource.top.mopp.TopTaskItem> foundItems = new java.util.ArrayList<org.emftext.language.top.resource.top.mopp.TopTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.top.resource.top.mopp.TopTaskItem(keyword, message, localLine, offset, end));
